...
A question where randomization is done  by creating an array of *all* possible values  for the sample space and having WeBWorK  select  one of the value. The `randomizer` function can  be   used to share this random selection amongst questions.
"""
...
Type for output only things (Plots, hints, ...)
"""
...
The  choices questions don't  readily lend themselves to fit  with  the `AbstractRandomizedQ` setup,  so these pass on to WeeBWorK for randomization.
"""
